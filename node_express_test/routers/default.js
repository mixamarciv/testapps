...
router.get('/phaser_test', rfnc.asyncRoute(async function(req, res){
	...
	{//phaser-kinetic-scrolling-plugin.min.js
		let phaserjsfile = path.join2(clientfilespath,'/phaser-kinetic-scrolling-plugin.min.js');
		let b = fs.existsSync(phaserjsfile);
		if(!b){
			let srcfile = path.join2(clientfilespath,
				'../node_modules/phaser-kinetic-scrolling-plugin/dist/phaser-kinetic-scrolling-plugin.min.js');
			wlogf('copy phaser-ce file from: %s to: %s',srcfile,phaserjsfile);
			fs.copyFileSync(srcfile,phaserjsfile);
		}
	}
